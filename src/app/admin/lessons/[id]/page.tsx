...
export default function EditLessonPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setDirections(dirs);
      const lesson = lessons.find((l: { id: string }) => l.id === id);
      if (lesson) {
        setName(lesson.name);
        setDirectionId(lesson.directionId);
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/lessons", {
        ...
        body: JSON.stringify({ id, name, directionId }),
      });
      if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-stone-100">
      ...
      <main className="max-w-md mx-auto px-4 py-8">
        <h1 className="font-display text-2xl font-bold text-stone-900 mb-6">Darsni tahrirlash</h1>
